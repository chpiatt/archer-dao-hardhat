...
contract PrismProxy {
    ...
    bytes32 constant PRISM_PROXY_STORAGE_POSITION =
        keccak256("prism.proxy.storage");
     ...
     */
    function setPendingProxyImplementation(address newPendingImplementation)
        public
        returns (bool)
    {
        ...
        require(
            msg.sender == s.admin,
            "Prism::setPendingProxyImp: caller must be admin"
        );
    ...
    function acceptProxyImplementation() public returns (bool) {
        ...
        require(
            msg.sender == s.pendingImplementation &&
     ...
     * @return boolean indicating success of operation
     */
    function setPendingProxyAdmin(address newPendingAdmin)
        public
        returns (bool)
    {
        ProxyStorage storage s = proxyStorage();
        ...
        require(
            msg.sender == s.admin,
            "Prism::setPendingProxyAdmin: caller must be admin"
        );
